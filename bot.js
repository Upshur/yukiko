...
client.on("guildMemberAdd", async member => {
  const channel = member.guild.channels.find('name', '🎫-mod-log-🎫'); //Giris cikis kanalini yazin!
  if (!channel) return;
...
client.on("guildMemberRemove", async member => {
  const channel = member.guild.channels.find('name', 'GIRIS CIKIS KANALININ TAM ISMINI YAZIN'); //Giris cikis kanalini yazin!
  if (!channel) return;
        let username = member.user.username;
        if (channel === undefined || channel === null) return;
        if (channel.type === "text") {            
